...
class StargateTracer(BridgeTracer):
    ...
    SWAP_TOPIC = "0x34660fc8af304464529f48a778e03d03e4d34bcd5f9b6f0cfbf3cd238c642f7f"
...
class AcrossTracer(BridgeTracer):
    ...
    EVENT_TOPIC = "0xa123dc29aebf7d0c3322c8eeb5b999e859f39937950ed31056532713d0de396f"
        ...
        for log in logs:
            if log.get("topics", [None])[0] == self.EVENT_TOPIC:
                try:
                    ...
                    # topics[3] = depositor. Recipient is non-indexed in data.
                    # data: inputToken(32), outputToken(32), inputAmount(32),
                    #       outputAmount(32), quoteTimestamp(32), fillDeadline(32),
                    #       exclusivityDeadline(32), recipient(32), ...
                    data = bytes.fromhex(log.get("data", "0x")[2:])
                    dst_chain_id = int(topics[1], 16) if len(topics) > 1 else 0
                    deposit_id   = int(topics[2], 16) if len(topics) > 2 else 0
                    depositor    = "0x" + topics[3][-40:] if len(topics) > 3 else "unknown"
                    recipient    = "0x" + data[224:256].hex()[-40:]
                    input_token  = "0x" + data[0:32].hex()[-40:]
                    output_token = "0x" + data[32:64].hex()[-40:]
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "tx":
        ...
        if results:
            output_path = PROJECT_ROOT / "artifacts" / "reports" / "bridge_trace_result.json"
